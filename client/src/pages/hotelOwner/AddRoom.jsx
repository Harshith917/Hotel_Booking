    ...
    <form>
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        ...
        {Object.keys(images).map((key)=>(
          ...
          // here key combines the string with the key like roomImages1
          <label htmlFor={`roomImages${key}`} key={key}>

            {/* //if image stored shows a temporary preview within that specified size and if not exists shows a default image */}
            <img className='max-h-13 cursor-pointer opacity-80' src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="" /> 

            
            <input type="file" accept='image/*' id={`roomImages${key}`} hidden onChange={(e)=>{
        ...
        <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
          <div className='flex-1 max-w-48'>
            ...
            <select value={inputs.roomType} onChange={e => setInputs({...inputs, roomType : e.target.value})} className='border opacity-70 border0gray-300 mt-1 rounded p-2 w-full'>
